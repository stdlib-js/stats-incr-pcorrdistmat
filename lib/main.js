...
function incrpcorrdistmat( out, means ) {
	...
	if ( isPositiveInteger( out ) ) {
		...
		throw new TypeError( format( '1JdDe', out ) );
	}
	...
	if ( arguments.length > 1 ) {
		if ( !isVectorLike( means ) ) {
			throw new TypeError( format( '1JdDc', means ) );
		}
		if ( numel( means.shape ) !== order ) {
			throw new Error( format( '1Jd9f', order, numel( means.shape ) ) );
		}
	...
	*/
	function accumulator1( v ) {
		...
		if ( !isVectorLike( v ) ) {
			throw new TypeError( format( '1JdDd', v ) );
		}
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1Jd9g', order, v.shape[ 0 ] ) );
		}
		n = N;
		N += 1;
		r = n / N;
	...
	*/
	function accumulator2( v ) {
		...
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1Jd9h', order, v.shape[ 0 ] ) );
		}
		N += 1;
		for ( i = 0; i < order; i++ ) {
			// Compute the residual:
