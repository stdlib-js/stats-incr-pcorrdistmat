...
function incrpcorrdistmat( out, means ) {
	...
	if ( isPositiveInteger( out ) ) {
		...
		throw new TypeError( format( '1JdI4', out ) );
	}
	...
	if ( arguments.length > 1 ) {
		if ( !isVectorLike( means ) ) {
			throw new TypeError( format( '1JdI0', means ) );
		}
		if ( numel( means.shape ) !== order ) {
			throw new Error( format( '1Jd9f,I5', order, numel( means.shape ) ) );
		}
	...
	*/
	function accumulator1( v ) {
		...
		if ( !isVectorLike( v ) ) {
			throw new TypeError( format( '1JdI2', v ) );
		}
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1Jd9g,I6', order, v.shape[ 0 ] ) );
		}
		n = N;
		N += 1;
		r = n / N;
	...
	*/
	function accumulator2( v ) {
		...
		if ( v.shape[ 0 ] !== order ) {
			throw new Error( format( '1Jd9h,I7', order, v.shape[ 0 ] ) );
		}
		N += 1;
		for ( i = 0; i < order; i++ ) {
			// Compute the residual:
