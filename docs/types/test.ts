...
import incrpcorrdistmat = require( './index' );
...
// The function returns an accumulator function...
{
	...
	incrpcorrdistmat( cov ); // $ExpectType accumulator
	incrpcorrdistmat( 2 ); // $ExpectType accumulator
	...
	means.set( 0, 3.0 );
	means.set( 1, -5.5 );
	incrpcorrdistmat( 2, means ); // $ExpectType accumulator
}

// The compiler throws an error if the function is provided a first argument which is not an ndarray or number...
{
	incrpcorrdistmat( '5' ); // $ExpectError
	incrpcorrdistmat( true ); // $ExpectError
	incrpcorrdistmat( false ); // $ExpectError
	incrpcorrdistmat( null ); // $ExpectError
	incrpcorrdistmat( undefined ); // $ExpectError
	incrpcorrdistmat( [] ); // $ExpectError
	incrpcorrdistmat( {} ); // $ExpectError
	incrpcorrdistmat( ( x: number ): number => x ); // $ExpectError
}

// The compiler throws an error if the function is provided a second argument which is not an ndarray...
{
	incrpcorrdistmat( 2, '5' ); // $ExpectError
	incrpcorrdistmat( 2, true ); // $ExpectError
	incrpcorrdistmat( 2, false ); // $ExpectError
	incrpcorrdistmat( 2, null ); // $ExpectError
	incrpcorrdistmat( 2, [] ); // $ExpectError
	incrpcorrdistmat( 2, {} ); // $ExpectError
	incrpcorrdistmat( 2, ( x: number ): number => x ); // $ExpectError
}
...
// The compiler throws an error if the function is provided an invalid number of arguments...
{
	...
	incrpcorrdistmat(); // $ExpectError
	incrpcorrdistmat( cov, means, {} ); // $ExpectError
}

// The function returns an accumulator function which returns an accumulated result...
{
	const acc = incrpcorrdistmat( 2 );
...
// The function returns an accumulator function which returns an accumulated result (known means)...
{
	...
	means.set( 0, 3.0 );
	means.set( 1, -5.5 );
	const acc = incrpcorrdistmat( 2, means );
